...
impl MediaWiki {
    pub fn new(country_code: &str, cache: Value) -> Self {
    ...
    pub async fn get_extract(&mut self, title: &str, hint: &MediaHint) -> Option<String> {
        ...
        let page_title = if let Some(page_title) = self.cache.get(&key) {
            page_title.to_owned()
        } else {
            let page_title = self.get_page_title(title, hint).await?;
        ...
        self.get_html_extract(&page_title).await
    }
    ...
    async fn get_html_extract(&self, title: &str) -> Option<String> {
        let api = mediawiki::api::Api::new(&self.api_url).await.unwrap();
        let params = api.params_into(&[
            ("action", "query"),
            ("prop", "extracts"),
...
mod tests {
    ...
    async fn artist_tests() {
        ...
        let mediawiki = MediaWiki::new("en", Value::Null);
